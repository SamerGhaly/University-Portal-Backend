...
const {courseNotInDepartment,courseNotFound,notInstructor,departmentDoesnotExist,entryAlreadyExist,catchError,entryNotExist}=require("../constants/errorCodes")
...
const deleteCourseInstructor=async(req,res)=>{
    ...
    try{
        const member=await Member.findOne({_id:req.body.instructorIdDeleted,type:memberRoles.INSTRUCTOR})
        if(!member)
        return res.status(404).json({
            message: ' not Instructor',
            code: notInstructor,
          })
           const member2=await Member.findOne({_id:req.body.instructorIdAdded,type:memberRoles.INSTRUCTOR})
        if(!member2)
        return res.status(404).json({
            message: ' not Instructor',
            code: notInstructor,
...
me.department.coursesPerDepartment.forEach(element => {
     if(element._id.toString()===req.body.courseId)
        flag=false
})
...
const out=await CourseAssignment.find({course:req.body.courseId}).select('member').populate('member',)
if(out.length==0)
 return res.json({
        code:   courseNotFound,
...
module.exports={addCourse,updateCourse,deleteCourse,assignCourseInstructor,deleteCourseInstructor,updateCourseInstructor,viewMemberInCourse}
